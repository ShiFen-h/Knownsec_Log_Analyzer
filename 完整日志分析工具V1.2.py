...
class LogAnalyzerApp:
                ...
                if os.path.exists(self.latest_downloaded_log_path):
                    try:
                        ...
                        if self.latest_downloaded_log_path.endswith('.gz'):
                            with gzip.open(self.latest_downloaded_log_path, 'rt', encoding='utf-8',
                                           errors='ignore') as f:
                                content = f.read()
                        else:
    ...
    def _validate_uploaded_log(self):
        ...
        # 在新线程中运行校验，以避免UI冻结
        validation_thread = threading.Thread(target=self._execute_upload_validation, args=(self.uploaded_log_content,),
                                             daemon=True)
    ...
    def create_custom_analysis_tab(self):
        ...
        time_filter_cb = ttk.Checkbutton(filter_frame, text="启用时间过滤", variable=self.enable_time_filter,
                                         command=self._on_time_filter_toggle)
        ...
        # 时间选择按钮
        self.start_time_btn = ttk.Button(filter_frame, text="选择", command=lambda: self._pick_time(self.start_time_var),
                                         state=tk.DISABLED)
        self.start_time_btn.pack(side=tk.LEFT, padx=2)
        self.end_time_btn = ttk.Button(filter_frame, text="选择", command=lambda: self._pick_time(self.end_time_var),
                                       state=tk.DISABLED)
        ...
        ttk.Radiobutton(match_mode_frame, text="包含", variable=self.match_mode, value="include").pack(side=tk.LEFT,
                                                                                                     padx=5)
        ttk.Radiobutton(match_mode_frame, text="不包含", variable=self.match_mode, value="exclude").pack(side=tk.LEFT,
                                                                                                      padx=5)
        ...
        ttk.Radiobutton(logic_frame, text="逻辑且(AND)", variable=self.logic_relation, value="and").pack(side=tk.LEFT,
                                                                                                      padx=5)
        ttk.Radiobutton(logic_frame, text="逻辑或(OR)", variable=self.logic_relation, value="or").pack(side=tk.LEFT,
                                                                                                    padx=5)
        ...
        self.custom_analyze_btn = ttk.Button(btn_frame, text="开始分析", command=self._start_custom_analysis,
                                             bootstyle=SUCCESS)
    ...
    def _start_custom_analysis(self):
        ...
        try:
            ...
            match_conditions = [line.strip() for line in self.match_text.get("1.0", tk.END).splitlines() if
                                line.strip()]
    ...
    def _show_log_samples(self, filepath, log_type_name="日志"):
        ...
        try:
            ...
            self._update_download_status(
                f"--- {log_type_name}: {os.path.basename(filepath)} (前{len(lines_to_show_top)}行) ---")
            ...
            if line_count > lines_to_display:
                self._update_download_status(
                    f"--- {log_type_name}: {os.path.basename(filepath)} (后{len(lines_to_show_bottom)}行) ---")
    ...
    def _start_process_uploaded_files(self):
        ...
        threading.Thread(
            ...
            args=(list(self.uploaded_file_paths), self.upload_merge_var.get(), self.upload_merge_name_var.get(),
    ...
    def _execute_upload_processing(self, file_paths, should_merge, merge_name, temp_dir):
        ...
        try:
            ...
            if should_merge and len(processed_files_for_merging) > 0:
                ...
                if self._merge_gz_files(processed_files_for_merging, merged_output_path,
                                        result_widget_name="upload_result_text", status_prefix="上传模块合并"):
                    # 显示合并后的文件大小
                ...
                self._update_upload_status(
                    f"单个文件处理完成: {os.path.basename(processed_log_path_for_validation)} (大小: {size_str})")
                ...
                if file_size < 1024:
                    ...
                    size_str = f"{file_size / (1024 * 1024 * 1024):.2f} GB"
                self._update_upload_status(
                    f"{len(processed_files_for_merging)} 个文件处理完成（未合并）。校验将针对: {os.path.basename(processed_log_path_for_validation)} (大小: {size_str})")
            ...
            if self.latest_uploaded_log_path and os.path.exists(self.latest_uploaded_log_path):
                ...
                if total_lines > 0:
                    self._update_upload_status(
                        f"在 {os.path.basename(self.latest_uploaded_log_path)} 中总共统计到 {total_lines} 条有效日期日志。")
                else:
    ...
    def download_logs(self, urls, save_dir, merge_var, merge_name):
        ...
        for i, url in enumerate(urls):
            ...
            if result:
                ...
                self._update_status(
                    f"下载结果: 成功 | 文件名: {os.path.basename(result)} | 文件大小: {self._format_size(file_size)} | 耗时: {elapsed_time:.2f}秒\n---")
            else:
